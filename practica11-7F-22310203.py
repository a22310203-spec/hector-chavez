...
class CNN_Reciclaje:
    def __init__(self):
        self.layers = [
            ...
            Dense(256, 4)  # 4 categorías de reciclaje
        ]
...
def fit(model, X_train, y_train, X_val, y_val, epochs=10, batch_size=128):
...
if __name__ == "__main__":
    # Cargar dataset CIFAR-10 (ejemplo)
    ...
    # Simular reciclaje con 4 clases (usamos las primeras 4 de CIFAR-10)
    ...
    y_train = np.vectorize(class_map.get)(y_train.flatten())
    y_test = np.vectorize(class_map.get)(y_test.flatten())
    ...
    model = CNN_Reciclaje()
    fit(model, X_train, y_train, X_val, y_val, epochs=10)
